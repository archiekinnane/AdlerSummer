from os_files import OScreator
...
def printArgs():
    print("OS_from_viz(viz_ID, file_name, extraCols = ['default'], GUI_name = 'default', GUI_path = 'default', use_redshift = 'OFF')")

def OS_from_viz(viz_ID, file_name, extraCols = ['default'], GUI_name = 'default', GUI_path = 'default', use_redshift = 'OFF'):
	...
	OScreator.createFiles(grbAstroCat, file_name, GUI_name, GUI_path)
...
def hasDistance(TableName):
    ...
    if 'DISTANCE' not in TableName.colnames and 'DIST' not in TableName.colnames:
        ...
        if manual_distance != '1':
            ...
            if use_redshift == 'ON' or use_redshift == 'on' or use_redshift == 'On':
            ...
            use_redshift = input("Redshift column also not found. If it exists, input name manually. Else, type 1 and all data will be placed at distance = 100 Mpc." )
            if use_redshift != '1':
                TableName.rename_column(use_redshift, 'Z')
                colNames.append('Z')
            else:
                good = 1
